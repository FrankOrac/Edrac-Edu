...
function requireTransportManagerOrAdmin(req: Request, res: Response, next: () => void) {
...
// Create transport
router.post('/', requireTransportManagerOrAdmin, async (req: Request, res: Response) => {
...
router.get('/:id', async (req: Request, res: Response) => {
  ...
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid transport ID' });
  try {
...
// Update a transport
router.put('/:id', requireTransportManagerOrAdmin, async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const { route, driver, vehicle, capacity, available } = req.body;
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid transport ID' });
  try {
...
// Delete a transport
router.delete('/:id', requireTransportManagerOrAdmin, async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid transport ID' });
  try {
