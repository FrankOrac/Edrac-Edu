...
function requireAdminOrPluginManager(req, res, next) {
...
// Create plugin
router.post('/', requireAdminOrPluginManager, async (req, res) => {
...
// Update a plugin
router.put('/:id', requireAdminOrPluginManager, async (req, res) => {
    const id = Number(req.params.id);
    const { name, description, enabled } = req.body;
    if (isNaN(id))
        return res.status(400).json({ error: 'Invalid plugin ID' });
...
// Delete a plugin
router.delete('/:id', requireAdminOrPluginManager, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ error: 'Invalid plugin ID' });
    try {
...
router.post('/:id/toggle', requireAdminOrPluginManager, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ error: 'Invalid plugin ID' });
    try {
