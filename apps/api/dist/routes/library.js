...
function requireLibrarianOrAdmin(req, res, next) {
...
// Add library item
router.post('/', requireLibrarianOrAdmin, async (req, res) => {
...
router.put('/:id', requireLibrarianOrAdmin, async (req, res) => {
    const id = Number(req.params.id);
    const { title, author, isbn, type, available } = req.body;
    if (isNaN(id))
        return res.status(400).json({ error: 'Invalid library item ID' });
...
router.delete('/:id', requireLibrarianOrAdmin, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ error: 'Invalid library item ID' });
    try {
