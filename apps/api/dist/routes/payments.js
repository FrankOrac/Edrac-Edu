...
const flw = new Flutterwave(process.env.FLUTTERWAVE_PUBLIC_KEY || 'FLWPUBK_TEST-dummy', process.env.FLUTTERWAVE_SECRET_KEY || 'FLWSECK_TEST-dummy');
...
router.post('/offline/approve/:id', auth, requireFinanceOrAdmin, async (req, res) => {
...
router.post('/offline/reject/:id', auth, requireFinanceOrAdmin, async (req, res) => {
    const id = Number(req.params.id);
    try {
        const payment = await prisma.payment.update({
            where: { id },
...
router.get('/analytics', auth, requireFinanceOrAdmin, async (req, res) => {
    try {
        const payments = await prisma.payment.findMany();
