...
const nigeria_subjects_1 = require("../data/nigeria-subjects");
const router = (0, express_1.Router)();
// Get all subjects
router.get('/', async (req, res) => {
    try {
        const { level, category, examType } = req.query;
        let subjects = nigeria_subjects_1.nigeriaSubjects;
        if (level) {
            subjects = (0, nigeria_subjects_1.getSubjectsByLevel)(level);
        }
        if (category) {
            subjects = (0, nigeria_subjects_1.getSubjectsByCategory)(category);
        }
        if (examType) {
            subjects = (0, nigeria_subjects_1.getSubjectsByExamType)(examType);
        }
        res.json({
            success: true,
            subjects: subjects
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const subject = nigeria_subjects_1.nigeriaSubjects.find(s => s.id === id);
        if (!subject) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existingSubject = nigeria_subjects_1.nigeriaSubjects.find(s => s.id === id);
        if (!existingSubject) {
            return res.status(404).json({
                success: false,
                error: 'Subject not found'
